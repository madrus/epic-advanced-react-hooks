...
`useDebugValue` hook can be very helpful when debugging hooks. We can calculate certain values and display them in the React DevTools as a part of the hook label. We can do that in a simple way by putting any string as the argument value:

```js
React.useDebugValue(`\`${query}\` => ${state}`)
```
or by providing a formatting function:
