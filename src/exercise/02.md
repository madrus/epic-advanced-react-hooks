...
The `run` function will be memoized with `useCallback` inside the hook. The nice thing is that we don't need any dependencies in the list of dependencies as `dispatch` is the only "variable" that is not being passed as an argument but it cannot change.
