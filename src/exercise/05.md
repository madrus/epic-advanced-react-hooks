...
`useImperativeHandle` is useful when we want to attach imperative methods like `scrollToTop` and `scrollToBottom` on `ref` that is passed in from the parent component via `React.forwardRef` so that the parent component gets access to triggering of those methods on a child component at will.
