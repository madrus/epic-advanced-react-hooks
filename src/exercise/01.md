...
1. the second argument to our reducer function is the `action`.  that is whatever argument is passed to our updater function.
2. `dispatch` function accepts an `action` as its argument and this action is whatever argument our `setState` function is being called with.
3. _reducer_ is a function that is used by `useReducer` and by the JavaScript `reduce` function as its first argument
